...
function plugin_smartredirect_install() {
	...
	if (! TableExists ( "glpi_plugin_smartredirect_configs" )) {
		/*	table pour stoquer la config du plugin
			id => utilisateur dont la config est sauvée
			...
			nothing => profiles_id pour les tickets sur lesquels on n'a pas de rôle
		...
		*/
		...
		$query = "INSERT INTO `glpi_plugin_smartredirect_preferences` 
					(`id`)
                	VALUES (0)";
...
function plugin_smartredirect_uninstall()
{
	...
	$tables = array("glpi_plugin_smartredirect_preferences");
